...
# GPS NEO-6M Serial Port (UART3 on Pin 29 -> /dev/ttyAMA3)
GPS_UART_PORT = "/dev/ttyAMA3"
GPS_BAUD = 9600

DHT_INTERVAL = 2.5
MPU_INTERVAL = 0.1       # Throttle to 10Hz to prevent I2C bus starvation
TILT_THRESHOLD = 5.0
...
# GPS NEO-6M Serial Port (/dev/ttyAMA3)
try:
...
def main():
    ...
    current_lcd_page = 0  # 0: Vitals, 1: Motion/Tilt, 2: GPS Position
    ...
    try:
        while True:
            ...
            # Read live GPS from /dev/ttyAMA3
            update_gps()
            ...
            if now - last_lcd_update > 0.4:
                if current_lcd_page == 0:
                    # Page 0: Heart Rate, SpO2, Temperature, Humidity, Sound
                    if ir < 20000:
                        line1 = "Place Finger..."
        ...
        # 3. Release GPIOs and serial ports
        if dht_device:
            dht_device.exit()
        if lora_serial and lora_serial.is_open:
            lora_serial.close()
