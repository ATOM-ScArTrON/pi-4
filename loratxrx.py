...
# Sensors & Hardware Switches (Strict debounce + active low pull-up)
...
# Waveshare LoRa HAT Control (M0=0, M1=0 -> Transparent Mode)
...
try:
    ...
    print("Waveshare LoRa HAT Serial connected on", LORA_UART_PORT)
...
# =========================================================
# 3. VOSK INITIALIZATION WITH STRICT GRAMMAR
# =========================================================
...
# Strict dictionary: forces Vosk to match ONLY these exact words and discard background noise
ALLOWED_COMMANDS = '["send", "receive", "come", "capture", "yes", "[unk]"]'
recognizer = KaldiRecognizer(model, SAMPLE_RATE, ALLOWED_COMMANDS)
print("Vosk model initialized with constrained vocabulary.")
...
# =========================================================
# 4. ACTIONS (TRIGGER-LOCKED EXECUTION)
# =========================================================
last_capture = 0
last_lora_tx = 0
last_lora_rx = 0
...
def send_lora_packet(display_bpm, display_spo2, source="ACTION"):
    global last_lora_tx
    current_time = time.time()
...
def receive_lora_packet(source="ACTION"):
    ...
    print(f"\n[LoRa RX] Manual trigger active via {source}...")
    ...
    # Clear old garbage from serial input before reading
    lora_serial.reset_input_buffer()
    ...
    if received_data:
        ...
        print("[LoRa RX] No packet detected within timeout.")
...
def main():
    ...
    print("\n--- SYSTEM OPERATIONAL ---")
    print("Voice Commands : 'send' (TX) | 'receive' or 'come' (RX) | 'capture' or 'yes' (Photo)")
    print("GPIO Buttons   : Pin 20 (TX) | Pin 16 (RX) | Pin 21 (Photo)")

    try:
        while True:
            # --- 1. Hardware Buttons (Explicit Is_Pressed Checks) ---
            if photo_switch.is_pressed:
                capture_photo(source="GPIO21 BUTTON")
                last_lcd_update = time.time()

            elif lora_tx_switch.is_pressed:
                send_lora_packet(display_bpm, display_spo2, source="GPIO20 BUTTON")
                last_lcd_update = time.time()
            ...
            if audio_data:
                if recognizer.AcceptWaveform(audio_data):
                    ...
                    if text and text != "[unk]":
                        ...
                        if "send" in tokens:
                            send_lora_packet(display_bpm, display_spo2, source="VOICE 'SEND'")
                            last_lcd_update = time.time()
