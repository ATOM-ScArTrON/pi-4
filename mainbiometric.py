...
# GPS NEO-6M Serial Port (UART3 -> /dev/ttyAMA3)
GPS_UART_PORT = "/dev/ttyAMA3"
GPS_BAUD = 9600

DHT_INTERVAL = 2.5
MPU_INTERVAL = 0.1       # 10Hz throttle
GPS_INTERVAL = 0.2       # 5Hz throttle
...
# GPS Global State & Throttled Reader
latest_lat = "0.0000"
latest_lon = "0.0000"
latest_sats = "0"
gps_has_fix = False
...
def update_gps():
    ...
    try:
        # Read available lines quickly without blocking the main loop
        for _ in range(5):
...
# MAX30102 Pulse Oximeter Initialization
def init_max30102():
    try:
...
ALLOWED_COMMANDS = '["send ok", "receive ok", "come ok", "capture ok", "yes ok", "go ok"]'
...
# Dedicated Real-Time Audio Worker Thread
def audio_listener():
    ...
    while not stop_threads.is_set():
        data = proc.stdout.read(800)  # Low buffer read for immediate responsiveness
        if not data:
            continue
        ...
        if recognizer.AcceptWaveform(data):
            ...
            ptext = pres.get("partial", "").strip().lower()
            tokens = ptext.split()
            # Fast trigger on partial recognition
            if any(cmd in tokens for cmd in ["send ok", "go ok", "receive ok", "come ok", "capture ok", "yes ok"]):
                voice_command_queue.put(ptext)
                recognizer = KaldiRecognizer(model, SAMPLE_RATE, ALLOWED_COMMANDS)
...
def send_lora_packet(display_bpm, display_spo2, pitch=0.0, roll=0.0, source="ACTION"):
    global last_lora_tx
    current_time = time.time()
    if current_time - last_lora_tx < 3:
        return
...
def receive_lora_packet(source="ACTION"):
    global last_lora_rx
    current_time = time.time()
    if current_time - last_lora_rx < 3.1:
        return
    ...
    while time.time() - start_wait < 3.8:
...
def main():
    ...
    try:
        while True:
            # 1. Read motion and GPS (throttled)
            pitch, roll, dir_fb, dir_lr = get_tilt_angles()
            update_gps()

            # 2. Check instant voice commands from the background queue
            while not voice_command_queue.empty():
                text = voice_command_queue.get_nowait()
                tokens = text.split()
                print(f"INSTANT VOICE COMMAND: '{text}'")
